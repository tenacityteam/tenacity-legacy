#ifndef __TENACITY_EXCEPTION__
#define __TENACITY_EXCEPTION__
 ...
 */
class TenacityException /* not final */
{
public:
    TenacityException() {}
    virtual ~TenacityException() = 0;
...
protected:
    ...
    TenacityException(const TenacityException &) = default;
...
public:
    explicit SimpleMessageBoxException(
        ...
        : MessageBoxException{exceptionType, caption}, message{message_} {
        helpUrl = helpUrl_;
    }
...
struct SimpleGuard<bool> {
    ...
    static SimpleGuard Default() { return SimpleGuard{false}; }
    const bool m_value;
};
...
template <typename R>
SimpleGuard<R> MakeSimpleGuard(R value) { return SimpleGuard<R>{value}; }
...
template <
    ...
    typename F3 =
        DefaultDelayedHandlerAction // Any( TenacityException * ), ignore return
    >
